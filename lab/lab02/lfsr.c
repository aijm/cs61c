...
void lfsr_calculate(uint16_t *reg) {
  ...
  /* YOUR CODE HERE */
	uint16_t reg_0 = (*reg) & 0x1u;
	uint16_t reg_2 = ((*reg) >> 2) & 0x1u;
	uint16_t reg_3 = ((*reg) >> 3) & 0x1u;
	uint16_t reg_5 = ((*reg) >> 5) & 0x1u;
	uint16_t shift_in = ((reg_0^reg_2)^reg_3)^reg_5;
	*reg = *reg >> 1; // right shift 1 bit
	*reg = (*reg) | (shift_in << 15); // shift in
